...
let handler = async(m, {conn, text, command, usedPrefix}) => {
    ...
    try {
        ...
        let noUrut = 1;
        for (let i of data ) {
            teks += `\n[${noUrut}]`+ global.ytSearch.replace("%title", i.title).replace("%view", i.views.toLocaleString('id-ID')).replace("%author", i.author.name).replace("%durasi", i.duration.timestamp).replace("%upload", i.ago).replace("%link", i.url)
            noUrut++;
        }
